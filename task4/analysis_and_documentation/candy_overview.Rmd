...
- I noticed in the data that there were a lot of different types of m&m's and I decided not to group them together as they are a stand alone packet of candy. I thought it would then be interesting to find out what m&m's got the most `ratings` and also what brought the most **joy**.
...
```
- We can see here that peanut m&m's (m_ms_peanut) has the most `ratings` and also the highest `joy_vs_despair_rating`.
...
- I then thought the only way to find a fair representation of people's feelings towards m&m's was to work out what proportion of each type brought `joy`. Here we can see that 78% of the Peanut M&M's brought joy, confirming their place as the most favourite M&M. This also showed us that public opinion is very much divided on the Mint M&M's, with 49% brining `joy`.
...
- Using the `rating system` to calulate the `joy_vs_despair_rating` for each type of M&M we can see that the Mint M&M's must have brought a lot of *despair* to lower their `joy_vs_despair_rating` to -88.
...
bb_candy_most_rated_m_ms <- bb_candy_bind_rating_system %>% 
  ...
  filter(m_ms == TRUE) %>% 
  group_by(candy) %>% 
  summarise(m_ms_ratings = n(), joy_vs_despair_rating = sum(rating_no)) %>% 
  arrange(desc(m_ms_ratings))
bb_candy_most_rated_m_ms
```

# ```{r}
# bb_candy_mean_age_favs <- bb_candy_bind_rating_system %>% 
#   filter(going_trick_or_treating == "Yes") %>% 
#   drop_na(age) %>% 
#   mutate(less_than_20 = age > 60) %>%
#   group_by(candy, less_than_20) %>% 
#   summarise(joy_vs_despair_rating = sum(rating_no)) %>% 
#   filter(less_than_20 == TRUE) %>% 
#   arrange(desc(joy_vs_despair_rating))
#   
#   bb_candy_mean_age_favs
